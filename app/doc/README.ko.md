...
게임 엔진에서 코드는 작업의 작은 일부일 뿐입니다. 나머지는 에셋과 파이프라인입니다 — 머티리얼, 블루프린트, 지형, 하늘, UI, 스켈레탈 애니메이션, 패키징, 성능. UltraGameStudio는 Claude Code / Codex / Gemini 계열의 코딩 에이전트를 이 현실에 맞춰 다시 만든 것입니다. 게임 엔진 개념을 이해하고, 게임에 필요한 모든 에셋(이미지, 3D 모델, 2D 스프라이트 애니메이션, 아틀라스, 오디오, 리깅, 비디오)을 생성하며, 반복 작업은 무료 또는 저비용 채널로 라우팅해 프리미엄 할당량을 중요한 곳에 씁니다.

<p align="center">
  <strong>원클릭 Unreal Engine UMG 인터페이스</strong><br>
  <img src="images/game/JMsXEKE.png" alt="UltraGameStudio가 원클릭으로 Unreal Engine UMG 인터페이스를 생성" width="960">
</p>

<p align="center">
  <strong>원클릭 3D 모델 생성</strong><br>
  <img src="images/game/noYfqPt.png" alt="UltraGameStudio가 원클릭으로 3D 모델을 생성" width="960">
  <br><br>
  <img src="images/20260615-214236.jpg" alt="UltraGameStudio가 생성한 3D 모델 미리보기" width="960">
</p>

<p align="center">
  <strong>이미지·스프라이트·메시·오디오·리깅·비디오 — 하나의 코딩 에이전트로 관리</strong><br>
  <img src="images/game/gmclmLS.png" alt="UltraGameStudio 통합 게임 에셋 생성" width="960">
  <br><br>
  <img src="images/game/jXgMffC.png" alt="UltraGameStudio 게임 에셋 워크플로" width="960">
</p>

## UltraGameStudio가 필요한 이유
...
UltraGameStudio는 Claude Code / Codex / Gemini 계열 에이전트를 게임 개발에 맞춰 깊이 커스터마이즈합니다.
...
UltraGameStudio에는 40개 이상의 게임 개발 전문가가 내장되어 작업에 따라 에이전트가 자동으로 호출합니다.
...
<p align="center">
  ...
  <img src="images/hero-free-channels.ko.png" alt="UltraGameStudio 무료 채널 라우팅 스크린샷" width="960">
</p>
...
복잡한 다단계 프로그래밍 작업의 경우, `/studio <작업>` 이 즉시 전용 실행 하네스를 생성하고 바로 실행합니다. 비주얼 캔버스가 필요하지 않습니다.
...
- 모든 실행은 `.ugs-run/<run-id>/` 아래에 전체 기록되며 작업 원장, 이벤트, 판정 및 최종 결과가 저장됩니다.
- 데스크톱 앱 또는 CLI에서 실행: `ugs studio "<작업>" --json --interactive --cwd <workspace>`.
...
`/studio` 사용 시, 하네스는 계획 단계 간에 여러 프로바이더를 자동으로 연결할 수 있습니다. 일반적인 패턴: DeepSeek이 저비용으로 초안을 생성하고, CodeX가 이어받아 최종 품질로 다듬습니다.

- **동적 하네스 계획**은 단계별 `model` 재정의를 지원 — 브레인스토밍/분류 단계에 DeepSeek, 구현/검증 단계에 CodeX/Gemini 할당.
- **cc-switch 호환성**: UltraGameStudio는 `cc-switch` CLI 구성을 읽어 Claude Code 라우팅용으로 구성된 모든 프로바이더를 studio 단계에서 즉시 사용 가능.
...
이 기능들은 개별 무료 프로바이더가 느리거나, 속도 제한 중이거나, 일시적으로 사용 불가능한 경우에도 `/studio` 하네스 실행의 복원력을 보장합니다.
...
4. UltraGameStudio에 key를 붙여넣고 **Save and Use**를 클릭합니다. 저장 후 경고 표시가 사라집니다.
...
4. 원하는 에셋을 설명합니다. UltraGameStudio는 먼저 프로그래밍 모델로 프롬프트를 다듬고, 설정된 Provider로 보냅니다.

<p align="center">
  <img src="images/生图/session-2026-06-07-2351.png" alt="에셋 모드는 같은 UltraGameStudio 세션 안에서 소재를 생성합니다" width="720">
</p>
