...
في محرك الألعاب، يمثل الكود جزءًا صغيرًا فقط من العمل. أما البقية فهي الأصول وخط الإنتاج: المواد والمخططات (blueprints) والتضاريس والسماء وواجهة المستخدم وتحريك الهياكل العظمية والتغليف والأداء. UltraGameStudio هو وكيل برمجة بأسلوب Claude Code / Codex / Gemini أُعيد بناؤه حول هذا الواقع: يفهم مفاهيم محركات الألعاب، ويولّد كامل مجموعة أصول اللعبة (صور، نماذج ثلاثية الأبعاد، تحريك sprites ثنائي الأبعاد، أطالس، صوت، rigging، فيديو)، ويوجّه العمل الروتيني عبر قنوات مجانية أو منخفضة التكلفة ليبقى رصيد النماذج المدفوعة لما يهم فعلًا.

<p align="center">
  <strong>واجهة UMG لـ Unreal Engine بنقرة واحدة</strong><br>
  <img src="images/game/JMsXEKE.png" alt="UltraGameStudio يولّد واجهة UMG لـ Unreal Engine بنقرة واحدة" width="960">
</p>

<p align="center">
  <strong>توليد نموذج ثلاثي الأبعاد بنقرة واحدة</strong><br>
  <img src="images/game/noYfqPt.png" alt="UltraGameStudio يولّد نموذجًا ثلاثي الأبعاد بنقرة واحدة" width="960">
  <br><br>
  <img src="images/20260615-214236.jpg" alt="معاينة النموذج ثلاثي الأبعاد الذي أنشأه UltraGameStudio" width="960">
</p>

<p align="center">
  <strong>الصور والـ sprites والـ meshes والصوت والـ rigging والفيديو — تُدار جميعها عبر وكيل برمجة واحد</strong><br>
  <img src="images/game/gmclmLS.png" alt="توليد أصول الألعاب الموحد في UltraGameStudio" width="960">
  <br><br>
  <img src="images/game/jXgMffC.png" alt="سير عمل أصول الألعاب في UltraGameStudio" width="960">
</p>

## لماذا UltraGameStudio
...
يأخذ UltraGameStudio وكيلًا بأسلوب Claude Code / Codex / Gemini ويخصصه بعمق لتطوير الألعاب:
...
يأتي UltraGameStudio مع أكثر من 40 متخصصًا في تطوير الألعاب يستدعيهم الوكيل تلقائيًا حسب المهمة:
...
<p align="center">
  ...
  <img src="images/hero-free-channels.ar.png" alt="لقطة شاشة لتوجيه القنوات المجانية في UltraGameStudio" width="960">
</p>
...
### سير العمل الديناميكي (/studio)

للمهام البرمجية المعقدة متعددة الخطوات، يقوم `/studio <مهمة>` بإنشاء هيكل تنفيذ مخصص فوراً وتشغيله مباشرة. لا حاجة للوحة مرئية.
...
- كل تشغيلة تُسجل بالكامل تحت `.ugs-run/<run-id>/` مع سجل المهام، والأحداث، والحكم، والنتيجة النهائية.
- شغّل من تطبيق سطح المكتب أو عبر CLI: `ugs studio "<مهمة>" --json --interactive --cwd <workspace>`.
...
مع `/studio`، يمكن للهيكل ربط مزودين متعددين عبر خطوات الخطة تلقائياً. النمط النموذجي: تنتج DeepSeek مسودات سريعة بتكلفة منخفضة، ثم تتولى CodeX التحسين للجودة النهائية.

- **خطة الهيكل الديناميكي** تدعم تجاوز `model` لكل خطوة — خصص DeepSeek للعصف الذهني/التصنيف وCodeX/Gemini للتنفيذ/التحقق.
- **توافق cc-switch**: يقرأ UltraGameStudio تكوين `cc-switch` CLI؛ أي مزود مكوّن لتوجيه Claude Code متاح فوراً لخطوات studio.
...
تضمن هذه الميزات مرونة تشغيل هيكل `/studio` حتى عندما يكون المزودون المجانيون الفرديون بطيئين أو محدودي المعدل أو غير متاحين مؤقتاً.
...
4. الصق المفتاح في UltraGameStudio واضغط **Save and Use**. بعد الحفظ تختفي علامة التحذير.
...
4. صف الأصل المطلوب. يطلب UltraGameStudio أولًا من نموذج البرمجة تحسين الـ prompt، ثم يرسله إلى المزود المضبوط.

<p align="center">
  <img src="images/生图/session-2026-06-07-2351.png" alt="وضع الأصول يولد عناصر داخل جلسة UltraGameStudio نفسها" width="720">
</p>
