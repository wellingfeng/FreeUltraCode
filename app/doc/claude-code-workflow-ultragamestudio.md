...
UltraGameStudio 做的是这件事：把 Claude Code 这类 workflows 做成可视化画布，再让同一份流程可以跑向 Claude Code、Codex、Gemini，以及更多本地或云端运行时。

下面按一个真实改动走：让 UltraGameStudio 支持多种界面风格，默认使用 Pencil，并且能在“设置 / 外观”里切换。

开发过程尽量放在 UltraGameStudio 里完成，顺便验证它能不能自举。
...
<p align="center">
  <img src="images/0-标题使用.png" alt="UltraGameStudio 主界面" width="960">
</p>

UltraGameStudio的主界面，中央是 workflows 蓝图，右侧是节点属性，底部是 AI 输入和返回。

UltraGameStudio的主界面大概分成四块：左侧是 workflows 历史，中央是可视化画布，右侧是节点属性和常用提示词，底部是 AI 输入与返回。

### 1. 下载 UltraGameStudio
...
```text
我希望为 UltraGameStudio 支持多种界面风格，
默认用 Pencil 来设计，
并且在“设置 / 外观”中可以切换。
```

写完后可以按 Ctrl+Enter 发送，也可以点右下角的发送按钮。UltraGameStudio会把这段自然语言转成一张可编辑的 workflows 蓝图。
...
发送需求后，UltraGameStudio会先把当前步骤整改成一个完整 workflows 。
...
这里其实也能说明 UltraGameStudio不是单纯画图。画布背后有统一的 workflows 结构，后面才能继续接不同运行时。
...
等待 UltraGameStudio开发完成后，重启程序，在“设置 / 外观”中点击切换不同风格外观。

截图里可以看到 Pencil、深邃午夜、极光、日光、余烬等风格卡片。选中某个风格后，会影响全局背景、面板、边框和运行状态颜色。

### 我觉得真正有用的地方

UltraGameStudio最有价值的地方，不是把 prompt 包了一层 UI。
...
workflows 这整个概念整体都还比较早，UltraGameStudio本身也还刚开始。运行时适配、节点能力和脚本生态都还会继续变。
...
上面讲的都是图形界面。但很多场景其实不需要画布——比如想把一个流程接进 CI、写进脚本、或者在服务器上无头跑。所以 UltraGameStudio 也提供了一个命令行版本，对应的 skill 叫 `/ultragamestudio`。

设计上我刻意把它做得很克制：**在用户这一侧，命令只有两个**。因为在命令行里，你不需要关心蓝图、IRGraph、编译这些中间概念——**workflow 对你来说就是一个 `.js` 脚本**，其余的转换全部自动发生。

```bash
ugs gen "做一个代码审查流程" -o review.js   # 用一句话生成一个 workflow 脚本
ugs gen review.js "再加一个安全审查节点"      # 用一句话修改已有脚本
ugs run review.js                          # 把脚本跑起来
```

就这三种用法（其实就 `gen` 和 `run` 两个命令）。

**`ugs gen`** 是用自然语言生成或修改 workflow。它和界面底部那个 AI 输入框是同一套能力：你说需求，它生成脚本；你指着已有脚本说怎么改，它改给你。

这里有个关键点：**它是零配置的，不需要你去填什么 API Key**。因为它复用的就是你本机已经登录好的 `claude` CLI（和运行时用的是同一条路），所以只要你装了 claude 并登录过，`ugs gen` 直接就能用。没装的话它会提醒你先 `claude login`。

**`ugs run`** 就是把脚本跑起来，逐个节点执行，终端里实时打印进度：
