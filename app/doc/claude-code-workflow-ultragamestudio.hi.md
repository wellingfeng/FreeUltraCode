# Claude Code में Dynamic Workflows आ गए हैं। बाकी मॉडलों का क्या? UltraGameStudio एक ओपन-सोर्स विकल्प है
...
इसी सोच के आधार पर मैंने UltraGameStudio बनाया, या अधिक सही कहें तो AI ने बनाया। यह Claude Code जैसे workflows को visual canvas में बदलता है और कोशिश करता है कि वही flow Claude Code, Codex, Gemini और दूसरे local या cloud runtimes पर भी चल सके।

इस बार मैं अमूर्त अवधारणाओं की बात नहीं करूंगा। सीधे screenshots के हिसाब से चलूंगा। उदाहरण भी ठोस है: UltraGameStudio को कई interface styles का समर्थन देना, Pencil को default बनाना, और Settings / Appearance में style बदलने देना।

विकास के दौरान मैंने अधिक से अधिक काम UltraGameStudio के अंदर ही करने की कोशिश की, ताकि यह खुद को bootstrap कर सके।
...
<p align="center">
  <img src="images/0-标题使用.png" alt="UltraGameStudio main interface" width="960">
</p>

UltraGameStudio के main interface में बीच में workflows blueprint है, दाईं ओर node properties हैं, और नीचे AI input और output है।
...
### 1. UltraGameStudio डाउनलोड करें
...
```text
मैं चाहता हूं कि UltraGameStudio कई interface styles support करे,
default में Pencil design इस्तेमाल करे,
और Settings / Appearance में इन्हें switch किया जा सके।
```

लिखने के बाद Ctrl+Enter दबा सकते हैं, या नीचे दाईं ओर send button दबा सकते हैं। UltraGameStudio इस natural language को editable workflows blueprint में बदल देता है।
...
Request भेजने के बाद UltraGameStudio पहले current step को एक complete workflow में reorganize करता है।
...
यह भी बताता है कि UltraGameStudio सिर्फ boxes draw नहीं कर रहा। Canvas के पीछे unified workflows structure है, इसलिए आगे अलग-अलग runtimes जोड़े जा सकते हैं।
...
UltraGameStudio development complete होने के बाद program restart करें, और Settings / Appearance में अलग-अलग appearance styles switch करें।
...
UltraGameStudio की सबसे बड़ी value prompt के ऊपर UI लगाने में नहीं है।
...
workflows का पूरा concept अभी काफी शुरुआती है, और UltraGameStudio भी अभी शुरू ही हुआ है। Runtime adapters, node capabilities और script ecosystem बदलते रहेंगे।
...
### Bonus: GUI नहीं खोलना चाहते? Command line पर बस दो commands काफी हैं

ऊपर तक जो बताया, वह सब graphical interface की बात थी। लेकिन कई scenarios में canvas की जरूरत ही नहीं होती — मान लीजिए आप किसी process को CI में जोड़ना चाहते हैं, script में embed करना चाहते हैं, या server पर headless चलाना चाहते हैं। इसी वजह से UltraGameStudio एक command line version भी देता है, और इससे जुड़ा skill है `/ultragamestudio`।

Design में मैंने इसे जान-बूझकर बहुत सीमित रखा है: **user की तरफ से commands सिर्फ दो हैं**। क्योंकि command line पर आपको blueprint, IRGraph, compilation जैसे intermediate concepts की चिंता करने की जरूरत नहीं है — **आपके लिए workflow का मतलब बस एक `.js` script है**, बाकी सारी conversions अपने-आप होती हैं।

```bash
ugs gen "एक code review process बनाओ" -o review.js   # एक वाक्य से workflow script generate करें
ugs gen review.js "इसमें एक security review node और जोड़ो"  # एक वाक्य से मौजूदा script modify करें
ugs run review.js                          # script को run करें
```

बस यही तीन उपयोग हैं (असल में commands दो ही हैं — `gen` और `run`)।

**`ugs gen`** natural language से workflow generate या modify करता है। यह interface के नीचे वाले AI input box जैसी ही capability है: आप requirement बताते हैं, यह script generate करता है; आप किसी मौजूदा script की तरफ इशारा करके बताते हैं कि क्या बदलना है, और यह बदल देता है।

यहां एक अहम बात है: **यह zero-config है, आपको कोई API Key नहीं भरना पड़ता**। क्योंकि यह आपके local machine पर पहले से login किए हुए `claude` CLI को ही reuse करता है (और runtime में भी यही path उपयोग होता है), तो जब तक आपके पास claude installed है और आप login हैं, `ugs gen` सीधे काम करता है। अगर installed नहीं है तो यह आपको पहले `claude login` करने की याद दिलाता है।

**`ugs run`** script को run करता है, node-by-node execute करता है, और terminal में real-time progress print करता है:
...
Parallel execution, pipeline, adversarial validation, automatic retry — ये सारे mechanisms interface पर "Run" दबाने के बिल्कुल same हैं — क्योंकि **command line और interface दोनों एक ही run kernel share करते हैं**, बस एक result को canvas पर draw करता है, दूसरा terminal पर print करता है।

कुछ काम के flags: `--dry-run` सिर्फ pre-check करता है, असली run नहीं करता (token बचाता है); `--resume` पिछली बार जहां fail हुआ था वहां से continue करता है; `--model` model specify करने के लिए है; `--json` machine-readable output देता है, ताकि pipelines में आसानी से जोड़ा जा सके।

एक वाक्य में इस command line का trade-off: **GUI "देखने और edit करने" के लिए है, command line "तेज़ चलाने और जोड़ने" के लिए है, लेकिन दोनों के पीछे एक ही workflow है।**
