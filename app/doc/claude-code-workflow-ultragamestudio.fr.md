# Claude Code a des Dynamic Workflows. Et les autres modèles ? Une alternative open source : UltraGameStudio
...
En suivant cette idée, j'ai développé UltraGameStudio, ou plus exactement, l'IA l'a développé. Il transforme les workflows de type Claude Code en canevas visuel et tente de faire en sorte qu'un même flux puisse cibler Claude Code, Codex, Gemini et d'autres runtimes locaux ou cloud.

Cette fois, je ne parle pas de concepts abstraits. Je parcours directement les captures d'écran. L'exemple est concret : faire en sorte qu'UltraGameStudio prenne en charge plusieurs styles d'interface, utilise Pencil par défaut et permette de basculer dans Paramètres / Apparence.

Pendant le développement, j'ai essayé de faire autant que possible dans UltraGameStudio pour qu'il puisse s'auto-amorcer.
...
<p align="center">
  <img src="images/0-标题使用.png" alt="Interface principale d'UltraGameStudio" width="960">
</p>

Dans l'interface principale d'UltraGameStudio, le blueprint des workflows est au centre, les propriétés des nœuds sont à droite, et l'entrée ainsi que la sortie IA sont en bas.
...
### 1. Télécharger UltraGameStudio

<p align="center">
  <img src="images/1-下载.png" alt="UltraGameStudio GitHub Releases" width="840">
</p>
...
Par défaut, UltraGameStudio utilise la CLI déjà configurée sur le système pour démarrer. Vous pouvez utiliser des outils comme CC-Switch pour la configurer.
...
```text
Je veux qu'UltraGameStudio prenne en charge plusieurs styles d'interface,
utilise Pencil comme design par défaut
et permette de changer dans Paramètres / Apparence.
```

Une fois le texte écrit, vous pouvez appuyer sur Ctrl+Enter ou cliquer sur le bouton d'envoi en bas à droite. UltraGameStudio transforme ce langage naturel en blueprint de workflows éditable.
...
Après l'envoi de la demande, UltraGameStudio réorganise d'abord l'étape actuelle en un workflow complet.
...
Cela montre aussi qu'UltraGameStudio ne se contente pas de dessiner des boîtes. Derrière le canevas se trouve une structure de workflows unifiée, ce qui permet ensuite de se connecter à différents runtimes.
...
Une fois le développement terminé par UltraGameStudio, redémarrez le programme et changez de style dans Paramètres / Apparence.
...
La vraie valeur d'UltraGameStudio n'est pas d'enrober un prompt dans une UI.
...
Le concept global de workflows est encore jeune, et UltraGameStudio lui-même vient de commencer. Les adaptateurs de runtime, les capacités des nœuds et l'écosystème de scripts continueront d'évoluer.
...
### En complément : pas envie d'ouvrir l'interface ? Deux commandes en ligne de commande suffisent

Tout ce qui précède concerne l'interface graphique. Mais dans bien des cas, on n'a pas besoin du canevas — par exemple lorsqu'on veut brancher un flux dans un pipeline CI, l'intégrer à un script, ou l'exécuter en mode headless sur un serveur. C'est pourquoi UltraGameStudio fournit aussi une version en ligne de commande, exposée via le skill `/ultragamestudio`.

J'ai volontairement gardé le design très sobre : **du côté de l'utilisateur, il n'y a que deux commandes**. Parce qu'en ligne de commande, vous n'avez pas à vous soucier du blueprint, de l'IRGraph ou de la compilation — **un workflow, pour vous, c'est juste un script `.js`**, et tout le reste se fait automatiquement.

```bash
ugs gen "crée un workflow de revue de code" -o review.js   # générer un script de workflow en une phrase
ugs gen review.js "ajoute un nœud d'audit de sécurité"      # modifier un script existant en une phrase
ugs run review.js                                           # lancer le script
```

Voilà les trois usages (en réalité, seulement deux commandes : `gen` et `run`).

**`ugs gen`** sert à générer ou modifier un workflow en langage naturel. Cela s'appuie exactement sur la même capacité que la zone de saisie IA en bas de l'interface : vous décrivez le besoin, elle produit le script ; vous pointez vers un script existant en disant ce qu'il faut changer, elle s'en occupe.

Un point clé ici : **c'est zéro configuration, vous n'avez pas à renseigner de clé API**. Parce que cela réutilise directement la CLI `claude` déjà connectée sur votre machine (le même chemin que celui utilisé à l'exécution). Donc, dès que vous avez installé claude et que vous êtes connecté, `ugs gen` fonctionne immédiatement. Sinon, il vous rappellera de faire d'abord `claude login`.

**`ugs run`**, c'est tout simplement lancer le script : les nœuds s'exécutent un à un, et la progression s'affiche en direct dans le terminal :
...
Parallélisme, pipeline, validation adversariale, réessai automatique : tous ces mécanismes sont identiques au clic sur « Exécuter » dans l'interface — parce que **la ligne de commande et l'interface partagent le même noyau d'exécution**. La seule différence, c'est que l'une dessine le résultat sur le canevas, et l'autre l'imprime dans le terminal.

Quelques options utiles : `--dry-run` pour ne faire qu'une pré-vérification sans exécuter (et économiser des tokens), `--resume` pour reprendre à partir du dernier nœud en échec, `--model` pour spécifier le modèle, et `--json` pour produire une sortie lisible par une machine, pratique pour s'intégrer à un pipeline.

En une phrase, le compromis de cette ligne de commande : **l'interface se charge de « voir et modifier », la ligne de commande se charge de « tourner vite et s'intégrer partout », mais derrière, c'est le même workflow.**

Groupe QQ : 149523963

Projet :

https://github.com/wellingfeng/UltraGameStudio
