...
UltraGameStudio turns Claude Code-style workflows into a visual canvas, then lets the same workflow target Claude Code, Codex, Gemini, and more local or cloud runtimes.

This walkthrough uses a real change: make UltraGameStudio support multiple interface styles, use Pencil by default, and allow switching in Settings / Appearance.

Most of the work happens inside UltraGameStudio itself, so the tool gets tested while it is being built.
...
The main UltraGameStudio interface has the workflows blueprint in the center, node properties on the right, and AI input and output at the bottom.
...
By default, UltraGameStudio uses the CLI already configured on your system to start. You can use tools such as CC-Switch to configure it.
...
```text
I want UltraGameStudio to support multiple interface styles,
use Pencil as the default design,
and allow switching in Settings / Appearance.
```

After writing it, press Ctrl+Enter or click the send button in the lower-right corner. UltraGameStudio turns this natural-language request into an editable workflows blueprint.
...
After sending the request, UltraGameStudio first reorganizes the current step into a complete workflow.
...
This also shows that UltraGameStudio is not just drawing boxes. The canvas is backed by a unified workflows structure, which is why it can later connect to different runtimes.
...
After UltraGameStudio finishes development, restart the program and switch between different appearance styles in Settings / Appearance.
...
The most valuable part of UltraGameStudio is not wrapping a prompt in a UI.
...
The whole concept of workflows is still early, and UltraGameStudio itself has only just started. Runtime adapters, node capabilities, and the script ecosystem will keep changing.
...
Everything above is the GUI. But plenty of situations don't actually need a canvas — you might want to wire a workflow into CI, drop it into a shell script, or run it headless on a server. So UltraGameStudio also ships a command-line version, exposed through a skill called `/ultragamestudio`.
...
```bash
ugs gen "build me a code-review workflow" -o review.js   # generate a workflow script from one sentence
ugs gen review.js "add a security-review node"           # modify an existing script with one sentence
ugs run review.js                                        # run the script
```

That's the whole surface area (really just two commands: `gen` and `run`).

**`ugs gen`** generates or modifies workflows from natural language. It's the same capability behind the AI input box at the bottom of the GUI: you describe what you want and it produces a script; you point at an existing script and describe a change, and it edits it for you.

One important detail: **it's zero-config — you don't have to plug in an API key**. It reuses the `claude` CLI that's already logged in on your machine (the same path the runtime uses), so as long as you have claude installed and authenticated, `ugs gen` just works. If it isn't installed, it'll nudge you to run `claude login` first.

**`ugs run`** executes a script, walking through it node by node and streaming progress to your terminal:
