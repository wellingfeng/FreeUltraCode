...
function handleStatement(stmt: Statement, ctx: ParseContext): void {
  // Self-contained runtime helpers (emitted with `// @ugs:runtime`, e.g. the
  ...
  if (readAnnotation(ctx.src, stmt, 'ugs:runtime')) return;
...
/** Remove the auto-injected `<!--ugs:ctx-->…<!--/ugs:ctx-->` data-flow block. */
