...
/** Strip the /gdd-mode-start|/gdd-mode-end command prefix from a chat line. */
export function stripGddModeCommand(text: string): string {
  return text
    .trim()
    .replace(/^\/gdd(?:-mode-(?:start|end))?\s*/iu, '')
    .trim();
}

// Game-design-document draft instruction for /gdd-mode. Draft turns keep design
// iteration cheap and reversible: update the GDD contract first, then freeze and
// implement only when /gdd-mode-end is submitted.
export function gddModePromptSystem(): string {
  return `你现在处于 GDD 草稿模式。目标是把用户的游戏想法反复收敛成可执行的游戏设计合同，而不是直接生成正式资产或写游戏实现代码。
核心规则：
- 先检测当前工作区项目引擎和已有设计文档；根据 .uproject、Packages/manifest.json + ProjectSettings、project.godot、project.json/assets、package.json 等标记判读 Unreal / Unity / Godot / Cocos / Web，不要默认使用 Godot。
- 优先维护可读 GDD 和机器可读清单；若项目已有 GDD 路径沿用它，否则建议使用 docs/gdd.md、docs/gdd.asset-manifest.json、docs/gdd.implementation-plan.json。
- GDD 必须记录 primary_template，优先从下方 Template Profile 注册表选择；用户没指定时按需求和当前引擎保守推断。
- 每轮允许反复修改玩法、世界观、关卡、角色、系统、经济、UI、音频、美术风格、资产清单和实现计划；保留用户已确认决策，显式记录新增/修改/删除影响。
- 草稿期不生成正式视觉/音频/视频资产，不写游戏运行时代码，不输出 UltraGameStudio workflow 蓝图或 IRGraph；用户明确要求预览/占位时，可生成轻量占位和说明。
- GDD 必须包含：核心体验、目标平台、视角/类型、核心循环、输入方式、场景/关卡、实体/角色、系统规则、失败/胜利条件、艺术方向、音频方向、UI/HUD、资产清单、实现计划、验收标准。
- asset_manifest 中每个资产要有稳定 key、类型、用途、规格、生成方式、依赖和验收标准；implementation_plan 要按当前引擎拆成可落地任务。
- 信息不足但不阻塞时直接做保守假设并标为“待确认”；确实阻塞时只输出完整 <<UGS_ASK>> 交互块。
${formatGameTemplateCatalogForPrompt('zh-CN')}
输出要求：
- 简体中文，结论先行。
- 汇报本轮 GDD diff、已更新文件/建议文件、剩余风险和下一步。
- 不要生成 workflow 蓝图，不要输出 IRGraph。`;
}

export function gddModeFinalizePromptSystem(): string {
  return `你现在要结束 GDD 模式。目标是冻结当前 GDD 修订，并按冻结合同开始正式落地资产与代码。
执行规则：
- 读取当前对话和工作区中已有 GDD/asset_manifest/implementation_plan；如果没有现成文件，先从对话上下文生成冻结版。
- 为冻结版生成清晰版本号或时间戳，记录与上一版的 diff：新增、修改、删除、影响资产、影响代码、风险。
- 校验核心循环、场景/关卡、实体、规则、UI、资产清单、实现计划和验收标准是否互相一致；发现矛盾先修正或列为阻塞。
- 提取 gameplay_contract、scene_plan、asset_manifest 和 implementation_plan；资产先于代码落地，代码必须引用真实或占位 asset key。
- 按 primary_template 校验模板能力、GDD 必填段、asset_manifest 槽位、代码 hook、浏览器验收是否闭合；缺模板时先从 Template Profile 注册表选择最小可行项。
- 只重跑受影响资产/代码；能复用则复用。缺少素材渠道时使用可替换占位资产并标注后续生成任务，不要虚构已生成文件。
- 按当前项目引擎落地：检测 Unreal / Unity / Godot / Cocos / Web 等项目标记，不要默认 Godot。
- 可执行时运行最相关的类型检查、构建、测试或启动校验；不能执行时说明具体原因。
- 正式验收必须包含浏览器检查：Web/Phaser/Vite/Cocos 直接启动 dev server 或静态 build 后用浏览器检查；Unity/Godot/Unreal 优先使用 WebGL/HTML harness/Pixel Streaming/可浏览预览。对应引擎的 play/build/compile 只能作为补充，不能替代浏览器验收。
- 不要生成 UltraGameStudio workflow 蓝图，不要输出 IRGraph。
${formatGameTemplateCatalogForPrompt('zh-CN')}
输出要求：
- 简体中文，结论先行。
- 汇报冻结版本、GDD diff、资产处理结果、代码处理结果、浏览器验证结果、引擎补充验证和剩余阻塞。`;
}
