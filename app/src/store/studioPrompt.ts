...
export interface ParsedStudioPrompt {
  request: string;
  options: Pick<
    StudioRunOptions,
    | 'concurrency'
...
export function parseStudioPrompt(input: string): ParsedStudioPrompt {
  const tokens = splitArgs(input);
  const requestTokens: string[] = [];
  const options: ParsedStudioPrompt['options'] = {};
...
export function studioModeLabel(
  options: ParsedStudioPrompt['options'],
): string {
  const modes: string[] = [];
...
export function summarizeStudioResult(result: StudioRunResult): string {
  const json = asObject(result.resultJson) as StudioJsonResult | null;
  ...
  appendEvidenceIndex(lines, json?.evidence);
...
export function studioAccepted(result: StudioRunResult): boolean {
...
function appendEvidenceIndex(lines: string[], rawEvidence: unknown): void {
  const evidence = arrayValue(rawEvidence).slice(0, 8) as EvidenceLike[];
  if (evidence.length === 0) return;
  lines.push('', '**证据索引**');
  for (const rawItem of evidence) {
    const item = asObject(rawItem);
    const kind = stringValue(item?.kind) ?? 'evidence';
    const status = stringValue(item?.status);
    const title = stringValue(item?.title) ?? stringValue(item?.id) ?? '未命名证据';
    const summary =
    ...
    const target = stringValue(item?.path) ?? stringValue(item?.command) ?? stringValue(item?.nodeId);
    lines.push(
      `- [${kind}]${status ? ` ${status}` : ''} ${clip(title, 56)}` +
        `${summary ? ` · ${clip(summary, 100)}` : ''}` +
        `${target && target !== summary ? ` · ${clip(target, 90)}` : ''}`,
    );
  }
}
...
function appendResumeAdvice(
  ...
  lines.push(
    `建议续跑: /studio --resume --run-id ${args.runId} --max-agent-calls ${suggestedCalls}${objective}`,
  );
}
