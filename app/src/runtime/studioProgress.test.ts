...
describe('studioProgress encode/decode', () => {
  it('round-trips a single event through a sentinel block', () => {
    const event: StudioProgressEvent = {
      kind: 'harness_ready',
      totalNodes: 7,
      maxAgentCalls: 12,
      objective: '修复登录流程',
  ...
  it('preserves surrounding prose and decodes events in order', () => {
    ...
    expect(cleaned).not.toContain('UGS_PROGRESS');
  ...
  it('leaves an incomplete trailing sentinel verbatim for the next chunk', () => {
    const partial = 'tail<<UGS_PROGRESS>>{"kind":"agent_calls","spent":1';
  ...
  it('drops malformed sentinel JSON silently', () => {
    const bad = '<<UGS_PROGRESS>>{not json}<<UGS_PROGRESS_END>>';
    const { events } = decodeProgressEvents(bad);
    expect(events).toEqual([]);
  });

  it('ignores unknown event kinds', () => {
    const unknown = '<<UGS_PROGRESS>>{"kind":"mystery"}<<UGS_PROGRESS_END>>';
