...
describe("dynamic harness", () => {
  it("normalizes planner JSON into a bounded harness spec", () => {
    ...
    expect(spec.strategies).toEqual([
      "loop-until-done",
  ...
  it("preserves omitted dependsOn as implicit previous-step dependency", () => {
    ...
    expect(
      graph.edges.some(
        (e) =>
          e.from.node === a?.id && e.to.node === b?.id && e.kind === "exec",
      ),
    ).toBe(true);
    expect(
      graph.edges.some(
        (e) =>
          e.from.node === LEDGER_NODE_ID &&
          e.to.node === b?.id &&
          e.kind === "exec",
      ),
    ).toBe(false);
  ...
  it("repairs ambiguous research plan dependencies into a decision chain", () => {
    const spec = parseDynamicHarnessSpec(
      JSON.stringify({
        objective: "执行多源调研，研究产品路线并输出建议",
        ...
        plan: [
          { id: "scope", kind: "agent", title: "Scope Freeze", dependsOn: [] },
          {
            id: "research",
            kind: "parallel",
            title: "Parallel Source Research",
            dependsOn: [],
          },
          {
            id: "synthesis",
  ...
  it("builds an executable ledger -> workers -> gate -> report graph", () => {
    ...
    expect(graph.nodes.find((node) => node.id === LEDGER_NODE_ID)?.type).toBe(
      "agent",
    );
    expect(graph.nodes.find((node) => node.id === WORKERS_NODE_ID)?.type).toBe(
      "parallel",
    );
    expect(graph.nodes.find((node) => node.id === GATE_NODE_ID)?.type).toBe(
      "consensus",
    );
    expect(
      graph.edges.some(
        (edge) =>
          edge.from.node === WORKERS_NODE_ID &&
          edge.to.node === GATE_NODE_ID &&
          edge.kind === "data",
      ),
    ).toBe(true);
  });
  ...
  it("expands loop-until-done into bounded repair rounds", () => {
    ...
    expect(graph.nodes.find((node) => node.id === "n_workers_r2")?.type).toBe(
      "parallel",
    );
    expect(graph.nodes.find((node) => node.id === "n_workers_r3")?.type).toBe(
      "parallel",
    );
    expect(graph.nodes.find((node) => node.id === "n_gate_r1")?.type).toBe(
      "consensus",
    );
    expect(graph.nodes.find((node) => node.id === "n_gate_r2")?.type).toBe(
      "consensus",
    );
    expect(graph.nodes.find((node) => node.id === GATE_NODE_ID)?.type).toBe(
      "consensus",
    ...
    expect(
      graph.edges.some(
        (edge) =>
          edge.from.node === "n_gate_r2" &&
          edge.to.node === "n_workers_r3" &&
          edge.kind === "exec",
      ),
    ).toBe(true);
    expect(
      graph.edges.some(
        (edge) =>
          edge.from.node === GATE_NODE_ID &&
          edge.to.node === REPORT_NODE_ID &&
          edge.kind === "exec",
      ),
    ).toBe(true);
  });
  ...
  it("compiles a dynamic plan into executable steps while preserving ledger and acceptance gate", () => {
    ...
    expect(
      graph.nodes.some(
        (node) => node.id.includes("inventory") && node.type === "agent",
      ),
    ).toBe(true);
    expect(
      graph.nodes.some(
        (node) => node.id.includes("solve") && node.type === "parallel",
      ),
    ).toBe(true);
    expect(
      graph.nodes.some(
        (node) => node.id.includes("verify") && node.type === "consensus",
      ),
    ).toBe(true);
    ...
    expect(
      graph.edges.some(
        (edge) =>
          edge.from.node === LEDGER_NODE_ID &&
          edge.to.node === solveNode?.id &&
          edge.kind === "data",
      ),
    ).toBe(true);
    expect(
  ...
  it("extracts ledger, verdict, and report artifacts from runtime outputs", () => {
    const artifacts = extractHarnessArtifacts({
      n_ledger: JSON.stringify({
        tasks: [
          { id: "t1", title: "复现", status: "accepted", acceptance: "有证据" },
        ],
      }),
  ...
  it("drives fan-out-and-synthesize on the PLAN path (production default path)", () => {
    ...
    expect(
      graph.edges.some(
        (e) =>
          e.from.node === research?.id &&
          e.to.node === "n_plan_synth" &&
          e.kind === "exec",
      ),
    ).toBe(true);
    expect(
  ...
  it("builds a planner graph with an escalated plan-critic stage", () => {
    ...
    expect(
      graph.edges.some(
        (e) =>
          e.from.node === PLANNER_NODE_ID &&
          e.to.node === PLAN_CRITIC_NODE_ID &&
          e.kind === "exec",
      ),
    ).toBe(true);
    expect(
    ...
    expect(
      graph.edges.some(
        (e) =>
          e.from.node === PLAN_CRITIC_NODE_ID &&
          e.to.node === "n_end" &&
          e.kind === "exec",
      ),
    ).toBe(true);
  ...
  it("prefers the critic's revisedSpec over the planner spec", () => {
    ...
    expect(resolved.spec.successCriteria).toEqual([
      "app/x.ts 存在且包含 export function foo",
    ]);
  ...
  it("normalizes objectiveChecks and drops unrunnable entries", () => {
    ...
    expect(spec.objectiveChecks?.map((c) => c.kind)).toEqual([
      "file-exists",
      "file-contains",
      "command",
    ]);
