...
} from '@/runtime/studioProgress';
import { t, type Locale } from '@/lib/i18n';

/**
 * Live run-progress card for a `/studio` run, modeled on Claude Code's
 ...
 * {@link StudioRunProgress} snapshot the store folds from the CLI's progress
 * sentinels, and renders ABOVE the run's human-readable log text (the log is
 * still shown by the caller).
 */
export default function StudioRunCard({
  progress,
  locale,
  active,
  onStop,
...
function NodeStatusIcon({ status }: { status: StudioNodeStatus }) {
