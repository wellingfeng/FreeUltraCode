/**
 * Pure run-summary derivation for `/studio` observability (phase 1 + 2).
 *
 ...
 *
 * This module is PURE: it derives everything from an StudioJsonResult, touches
 * no filesystem / runtime, and is unit-tested directly.
 */
import type { NodeRunResult, RunEvidence, RunNextStep } from '../../src/runtime';
...
export function buildRunSummary(json: StudioJsonResult): RunSummary {
  ...
  return {
    ...
    gate,
    evidence: (json.evidence ?? []).slice(0, 20),
    nextSteps: (json.nextSteps ?? []).slice(0, 12),
  };
}

/** Phase 2: cross model self-verdict against objective ground truth. */
export function classifyGate(json: StudioJsonResult): GateTruthQuadrant {
