...
import { buildRunSummary, classifyGate, nodeRole } from './studioSummary';
...
function baseResult(over: Partial<StudioJsonResult> = {}): StudioJsonResult {
  return {
    success: true,
    runId: 'uc-test',
    runDir: '/tmp/uc-test',
...
describe('buildRunSummary', () => {
  ...
  it('passes through the evidence index for trace rendering', () => {
    const s = buildRunSummary(
      baseResult({
        evidence: [
          {
    ...
    expect(s.evidence[0]).toMatchObject({ id: 'verify-command', kind: 'command' });
  });
  ...
  it('passes through evidence and next-step summaries', () => {
    const s = buildRunSummary(
      baseResult({
        success: false,
        evidence: [{ id: 'verify-command', kind: 'command', title: '验证命令' }],
        nextSteps: [{ id: 'rerun', title: '重跑', reason: '验证失败', command: 'ugs studio x' }],
      }),
    );
    expect(s.evidence[0].id).toBe('verify-command');
    expect(s.nextSteps[0].command).toBe('ugs studio x');
  });
});
